...
class UnscentedKalmanFilterNode:
    ...
    def state_transition(self, state, dt):
        x, y, z, roll, pitch, yaw, vx, vy, vz = state
        x += vx * dt
        y += vy * dt
        z += vz * dt
        return np.array([x, y, z, roll, pitch, yaw, vx, vy, vz])
    ...
    def fuse_sensors(self):
        ...
        try:
            ...
            # Integrate position
            self.position_x = self.ukf.x[0]
            self.position_y = self.ukf.x[1]
            ...
            odom.twist.twist.linear.x = self.ukf.x[6]
            odom.twist.twist.linear.y = self.ukf.x[7]
            odom.twist.twist.linear.z = self.ukf.x[8]
